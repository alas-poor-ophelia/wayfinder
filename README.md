...
Wayfinder supports **Path of War**, the third-party martial-maneuver system by Dreamscarred Press (base book and *Path of War: Expanded*). It is **off by default** — turn it on under **Settings → Wayfinder → Path of War**. Once enabled, an initiating character gains a **Maneuvers** tab that tracks readied maneuvers, stances, boosts, and strikes. Six initiating classes are supported: **Warlord, Warder, Stalker** (base) and **Harbinger, Mystic, Zealot** (Expanded), each with its own progression, save DCs, and recovery rule — including the Mystic's stochastic granted-maneuver cycling.

Effects fold into the sheet's math the way buffs and quick actions do: an active **stance** or **boost** applies its bonus automatically (scaling with Initiator Level where the rules say so), and a **strike** can be *armed* so its rider — extra damage, a to-hit bonus, or text like "knocked prone" — lands on your next attack and shows on the combat tab. Effects that aren't a clean numeric bonus (auras, healing, control) still render their text. A searchable **maneuver database** mirrors the spell database, with filters for discipline, type, tier, and **book of origin**, and maneuvers can be saved as loadouts. Turning the setting back off hides every Path of War surface and leaves your maneuver data untouched.
